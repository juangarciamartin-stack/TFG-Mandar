...
export const Lotes = () => {
  ...
  const handleBorrarPdf = async (idPliego) => {
    if (
      window.confirm(
        "¿Estás seguro de que deseas eliminar permanentemente este archivo PDF?",
      )
    ) {
      try {
  ...
  const handleSubmitLote = async (e) => {
    e.preventDefault();
    try {
      let miAyuntamientoId = "1";

      if (miRol === "Admin") {
        miAyuntamientoId = ayuntamientoSeleccionado;
      } else if (miRol === "Ayuntamiento") {

        const idAlmacenado = localStorage.getItem("idAyuntamiento");
        miAyuntamientoId =
          idAlmacenado && idAlmacenado !== "2" ? idAlmacenado : "1";
      }
    ...
    <div className="vista-page-container">
      ...
      <div className="vista-contenido-scroll">
        <div className="view-intro">
          ...
          <p>
            Gestión de bloques de licitación y presupuestos asignados por el
            municipio.
          </p>
        </div>
                        ...
                        {lote.pliegos &&
                          lote.pliegos.map((pliego) => (
                            ...
                            >
                              ...
                              <span
                                onClick={() => handleBorrarPdf(pliego.id)}
                                style={{
                                  color: "#ef4444",
                                  fontSize: "11px",
                                  cursor: "pointer",
            ...
            >
              {miRol === "Admin" && (
                <div>
                  ...
                  <select
                    value={ayuntamientoSeleccionado}
                    onChange={(e) =>
                      setAyuntamientoSeleccionado(e.target.value)
                    }
                    required
                    style={inputStyle}
                  >
        ...
        <div style={modalOverlayStyle}>
          <div style={modalContentStyle}>
            <h3 style={{ margin: 0, color: "#0f172a" }}>
              Subir Pliego de Condiciones
            </h3>
            <p style={{ fontSize: "13px", color: "#64748b", marginTop: "4px" }}>
              Asociando archivo técnico a:{" "}
              <strong>{loteParaPliego?.nombre}</strong>
            </p>

            <form
              onSubmit={handleSubmitPliego}
              style={{
                display: "flex",
                flexDirection: "column",
                gap: "15px",
            ...
            >
              <div>
                <label style={labelStyle}>
                  Seleccionar Fichero (Solo formato PDF)
                </label>
                <input
                  type="file"
                  accept=".pdf"
              ...
              >
                <button
                  type="button"
                  className="btn-vesta secundario"
                  onClick={() => setMostrarModalPliego(false)}
                >
                  Cancelar
                </button>
                <button
